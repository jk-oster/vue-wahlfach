...
function handleSendMessage() {
  sendMessage(newMessageText.value)
}
</script>
      ...
      <div class="overflow-y-auto flex-grow">
        <ul class="list bg-base-100 rounded-box shadow-md">
          ...
          <li class="p-4 pb-2 text-xs opacity-60 tracking-wide flex justify-between items-center">
            ...
            <div>
              <button class="btn btn-ghost btn-sm" onclick="add_modal.showModal()">
                Add chat
              </button>
            </div>
          </li>
          ...
          <li class="list-row cursor-pointer" v-for="chat in chats" :key="chat.id" @click="handleSetChat(chat.id)">
            <div>
              <img class="size-10 rounded-box" :src="'https://jet-escape.pockethost.io/api/files/chats/' + chat.id + '/' + chat.image"  alt="Chat Group Profile Image"/>
            </div>
            <div>
              <div>{{ chat.name }}</div>
